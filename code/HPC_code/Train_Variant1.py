...
def main():
    path = './Results/A/dense201_hirar/'
    img_path = img_path = './data/full/Yoga/YOGA_downloads/' 
    ...
    model = dense201_hirar()
    model.compile(loss=['categorical_crossentropy','categorical_crossentropy','categorical_crossentropy'], loss_weights=[1,1,1], optimizer= sgd, metrics=['accuracy'])
    model.set_weights(np.load('model_weights_V1.npy', allow_pickle=True))
    

    batch_size = 19
    test_steps_per_epoch = test_df.shape[0] // 19
