...
def process_data(df,img_path,train=True):
    ...
    for i in range(num):
        ...
        x1_label = PerfectTest_df['label of class_6'].iloc[i]
        x2_label = PerfectTest_df['label of class_20'].iloc[i]
        x3_label = PerfectTest_df['label of class_82'].iloc[i]
...
def main():
    path = './Results/C/dense201_hirar_new/'
    img_path = img_path = './data/cleaned/Yoga/YOGA_downloads_withoutClipart/' 

    
    train_df = PerfectTrain_df #TryTrain_df # 
    test_df = PerfectTest_df #TryTest_df # 
    ...
    np.savetxt('score.txt', score, fmt='%d')
    ###################################################################
