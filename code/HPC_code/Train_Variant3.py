...
def main():
    path = './Results/A/dense201_hirar_new/'
    img_path =  './data/full/Yoga/YOGA_downloads/' 
    ...
    np.save(path + 'model_weights_new.npy',model.get_weights())
    print('FIT ENDED')
    ...
    model = dense201_hirar_new()
    model.compile(loss=['categorical_crossentropy','categorical_crossentropy','categorical_crossentropy'], loss_weights=[1,1,1], optimizer= sgd, metrics=['accuracy'])#, 'top_k_categorical_accuracy'])
    model.set_weights(np.load(path + 'model_weights_new.npy', allow_pickle=True))
